"""
ENSTRECT LLM+RAG 损伤智能识别与报告生成 端到端Pipeline

用法:
    python run_llm_damage_report.py --images-dir path/to/images --output-dir output/

    # 不启用RAG（纯VLM推理）
    python run_llm_damage_report.py --images-dir path/to/images --no-rag

    # 指定模型路径和量化选项
    python run_llm_damage_report.py --model-path /path/to/qwen2.5-vl --no-4bit
"""
...
# Pipeline 类
# ============================================================
class DamageReportPipeline:
    """
    端到端损伤检测报告生成Pipeline

    流程: 图像输入 -> 分割 -> ROI裁剪 -> RAG检索 -> VLM推理 -> 报告输出
    """
    ...
    def _init_components(self):
        ...
        # 5. VLM 推理引擎
        if HAS_LLM_RAG:
            print(f"[VLM] 加载 Qwen2.5-VL (4bit={self.load_in_4bit})...")
            vlm_kwargs = {"load_in_4bit": self.load_in_4bit}
            if self.model_path:
                vlm_kwargs["model_name"] = self.model_path
            self.vlm = QwenVLInference(**vlm_kwargs)
            print("[VLM] ✓ 引擎就绪（模型将在首次推理时加载）")
            ...
            if self.enable_rag:
                ...
                json_path = str(PROJECT_ROOT / self.rag_json_path)
                db_path = str(PROJECT_ROOT / self.rag_db_path)
                if Path(json_path).exists():
                    ...
                    print(f"[RAG] ⚠ 规范JSON不存在: {json_path}, RAG禁用")
    ...
    def _segment_image(self, image_path: str) -> Tuple[Image.Image, np.ndarray, np.ndarray]:
        """对单张图像执行分割"""
        import torch
    ...
    def _process_single_image(self, image_path: Path) -> Dict:
        ...
        try:
            ...
            # Step 4: VLM推理
            print(f"  [4/5] VLM推理 ({len(rois)} 个ROI区域)...")
            multimodal_inputs = self.prompt_builder.build_multimodal_input(
                rois, geometry_stats, rag_contexts
            )

            reports_for_image = []
            for i, (text_prompt, roi_img) in enumerate(multimodal_inputs):
                cls_id = list(rois.keys())[i] if i < len(rois) else 0

                raw_output = self.vlm.generate([roi_img], text_prompt)
    ...
    def run(self) -> List[Dict]:
        """运行完整的批量处理流程"""
        print("\n" + "=" * 60)
        print("ENSTRECT LLM+RAG 损伤智能报告生成 Pipeline")
        print("=" * 60)
        ...
        image_files += sorted(self.images_dir.glob("*.jpeg")) + sorted(self.images_dir.glob("*.bmp"))
        # 去重
        seen = set()
        unique_files = []
        ...
        if self.enable_quality_filter and HAS_QUALITY_FILTER:
            print("[质量过滤] 检测图像质量...")
            try:
                filtered = filter_blurry_images([str(f) for f in image_files])
                image_files = [Path(f) for f in filtered]
                print(f"[质量过滤] 过滤后剩余 {len(image_files)} 张清晰图像")
            except Exception as e:
                print(f"[质量过滤] 过滤失败，使用全部图像: {e}")
...
def main():
    parser = argparse.ArgumentParser(
        description="ENSTRECT LLM+RAG 损伤智能报告生成 Pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python run_llm_damage_report.py --images-dir output/segments/bridge_b/segment_dev/views --output-dir output/
  python run_llm_damage_report.py --images-dir ./test_images --output-dir ./output --no-rag --max-images 3
        """
    )

    parser.add_argument("--images-dir", type=str, required=True, help="输入图像目录")
    parser.add_argument("--output-dir", type=str, default="output_llm_report", help="输出目录 (默认: output_llm_report)")
    parser.add_argument("--enable-rag", action="store_true", default=False, help="启用RAG规范检索")
    parser.add_argument("--no-rag", action="store_true", default=True, help="禁用RAG (默认)")
    parser.add_argument("--top-k", type=int, default=3, help="RAG检索返回条数 (默认3)")
    parser.add_argument("--model-path", type=str, default=None, help="Qwen2.5-VL模型路径 (默认使用官方权重)")
    parser.add_argument("--max-images", type=int, default=None, help="最大处理图像数 (默认全部处理)")
    parser.add_argument("--no-4bit", action="store_true", help="禁用4-bit量化 (使用FP16)")
    ...
    if not HAS_ENSTRECT:
        print("ENSTRECT 分割模块未安装，Pipeline无法执行分割步骤")
        print("请先确认 enstrect/src 目录存在且包含 nnunet_s2ds.py")
        sys.exit(1)

    # 创建并运行 Pipeline
    pipeline = DamageReportPipeline(
